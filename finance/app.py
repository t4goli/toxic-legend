...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO history (username, month, bos, date, year, company, nos, price) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", u, date.today().month, "bought", date.today().day, date.today().year, lookup(symbol)["symbol"], shares, money)
        return redirect("/")
    else:
